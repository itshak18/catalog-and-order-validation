...
function verifyYaadSignature(params: URLSearchParams, apiKey: string): boolean {
  ...
  const signatureString = sortedKeys.map((key) =>
    `${key}=${verifyParams.get(key)}`
  ).join("&");

  const crypto = require("crypto");
  const expectedSignature = crypto.createHmac("sha256", apiKey).update(
    signatureString,
  ).digest("hex");
...
function processYaadPayment(
  ...
  if (order.status === "paid") {
    ...
    console.warn(
      "Yaad notify received for already-paid order with different transaction",
      {
        orderId: order.id,
  ...
  if (status === "0") {
    ...
    if (!callbackAmount) {
      console.error(
        "Yaad notify: Success callback missing Amount field",
        {
          orderId: order.id,
          transactionId,
          payload: yaadPayload,
      ...
      return {
        success: false,
        error:
          "Payment success reported but amount is missing. Order marked failed for manual review.",
        idempotent: false,
      };
    }
    ...
    if (yaadAmount !== internalAmount) {
      ...
      return {
        success: false,
        error:
          `Amount mismatch: expected ${internalAmount}, received ${yaadAmount}`,
        idempotent: false,
      };
    }
    ...
    if (!result.success && !result.alreadyInState) {
      return {
        success: false,
        error: result.error || "Failed to mark order paid",
        idempotent: false,
      };
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!orderId) {
      console.error("Yaad notify: Missing order ID", {
    ...
    if (YAAD_API_KEY) {
      ...
      console.warn(
        "Yaad notify: No API key configured, skipping signature verification",
      );
      signatureValid = true;
    }
